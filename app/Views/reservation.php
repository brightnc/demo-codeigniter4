...
<head>
    ...
    <script>
        function handleSave(movie_detail_id, user_id, final_prices) {
            ...
            if (confirm("ยืนยันการจอง") == true) {
                $.ajax({
                    ...
                    data: "user_id=" + user_id + "&movie_detail_id=" + movie_detail_id + "&final_prices=" + final_prices,
...
<body>
    ...
    <table>
        ...
        for ($i = 0; $i < count($data_movies_list); $i++) {
            ...
            echo "<td><button id=btn-reservation-{$data_movies_list[$i]->movie_detail_id} onclick=handleSave('{$data_movies_list[$i]->movie_detail_id}','{$data_user_id}','{$final_prices}')>จอง</button></td>";
